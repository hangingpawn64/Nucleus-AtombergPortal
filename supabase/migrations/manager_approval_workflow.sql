...
-- ---------------------------------------------------------------------------
-- Role refactor: member/admin -> employee/manager/admin.
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- Manager hierarchy and review history.
-- ---------------------------------------------------------------------------
create table if not exists public.manager_relationships (
  ...
  constraint manager_relationships_not_self check (employee_id <> manager_id)
);

create unique index if not exists manager_relationships_one_active_employee_idx
on public.manager_relationships (employee_id)
...
create table if not exists public.goal_review_comments (
  id uuid primary key default gen_random_uuid(),
  goal_sheet_id uuid not null references public.goal_sheets(id) on delete cascade,
  author_id uuid references public.users(id) on delete set null,
  comment text not null,
  comment_type text not null default 'general'
    check (comment_type in ('general', 'approval', 'rework', 'unlock')),
  metadata jsonb not null default '{}'::jsonb,
  created_at timestamptz not null default now()
);

drop trigger if exists set_manager_relationships_updated_at on public.manager_relationships;
create trigger set_manager_relationships_updated_at
before update on public.manager_relationships
for each row execute function public.set_updated_at();

-- ---------------------------------------------------------------------------
-- Security helper functions used by RLS and workflow RPCs.
-- ---------------------------------------------------------------------------
  ...
  select exists (
    ...
    where gs.id = p_goal_sheet_id
      and (
        ...
        or public.is_active_manager_of(gs.employee_id)
        or public.is_admin()
      )
  )
$$;
...
-- ---------------------------------------------------------------------------
-- Manager assignment and locking triggers.
-- ---------------------------------------------------------------------------
...
begin
  if tg_op = 'INSERT' and new.manager_id is null then
    new.manager_id := public.active_manager_for_employee(new.employee_id);
...
drop trigger if exists set_goal_sheet_manager_before_write on public.goal_sheets;
create trigger set_goal_sheet_manager_before_write
before insert or update of employee_id, manager_id on public.goal_sheets
for each row execute function public.set_goal_sheet_manager();
...
begin
  ...
  set manager_id = public.active_manager_for_employee(target_employee)
...
drop trigger if exists sync_active_manager_to_goal_sheets_after_write on public.manager_relationships;
create trigger sync_active_manager_to_goal_sheets_after_write
after insert or update of manager_id, status, effective_to on public.manager_relationships
for each row execute function public.sync_active_manager_to_goal_sheets();
...
drop trigger if exists prevent_locked_goal_inserts on public.goals;
create trigger prevent_locked_goal_inserts
before insert on public.goals
for each row execute function public.prevent_locked_goal_edits();

drop trigger if exists prevent_locked_goal_updates on public.goals;
create trigger prevent_locked_goal_updates
before update on public.goals
for each row execute function public.prevent_locked_goal_edits();

drop trigger if exists prevent_locked_goal_deletes on public.goals;
create trigger prevent_locked_goal_deletes
before delete on public.goals
for each row execute function public.prevent_locked_goal_edits();

-- ---------------------------------------------------------------------------
-- Workflow RPCs. These centralize state transitions, notifications, and audit.
-- ---------------------------------------------------------------------------
...
begin
  ...
  v_manager_id := coalesce(v_sheet.manager_id, public.active_manager_for_employee(v_sheet.employee_id));
...
begin
  ...
  if not public.is_admin()
    ...
    and not public.is_active_manager_of(v_sheet.employee_id) then
...
using (
  id = auth.uid()
  or public.is_admin()
  or public.is_active_manager_of(id)
);
...
using (
  user_id = auth.uid()
  or public.is_admin()
  or public.is_active_manager_of(user_id)
);
...
using (
  actor_id = auth.uid()
  or public.is_admin()
  or public.is_active_manager_of(actor_id)
);
...
create policy "Authenticated users can view goal cycles"
...
using (
  employee_id = auth.uid()
  or manager_id = auth.uid()
  or public.is_active_manager_of(employee_id)
  or public.is_admin()
);
...
to authenticated
using (public.can_view_goal_sheet(goal_sheet_id));
...
on public.goal_checkins for all
to authenticated
using (
  exists (
    select 1
...
grant execute on function public.active_manager_for_employee(uuid) to authenticated;
grant execute on function public.is_active_manager_of(uuid) to authenticated;
grant execute on function public.can_view_goal_sheet(uuid) to authenticated;
